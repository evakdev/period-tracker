...
def create_cycle(date,dayobj):
    ...
    if id>0:
        ...
        a.save()
    return
def date_convert(strdate):  ####### To add string dates from postman. could also use datetime.strptime
    lst=strdate.split('/')
...
def signup(request):
    if request.method=='POST':
        ...
        if form.is_valid():
            form.save()
            return render(request,'test.html',{"msg":'Successful!'})
        return HttpResponse(f"{form.errors}")
    else:
        form=SignUpForm()
        return render(request,'signup.html',context={'signupform':form})
...
def tracking(request):
    if request.method=='POST':
        try:
            ...
            cnt=0
            if "blood" in thislog:
                for day in Day.objects.filter(date__gt=thisdate-datetime.timedelta(10)):
            ...
            return HttpResponse(
                f"<b> Successfully Saved!</b><br><br>{list_all()}")
...
def trackable_manage(request):
    ...
    if request.method=='GET':
        return render(request, 'trackables.html', context={'catdict':relateddict, 'done':"",'forms':forms})
    ...
    if request.method=="POST":
        if "addcategory" in request.POST:
            ...
            if form.is_valid():
                ...
                Trackable.objects.create(name=form.cleaned_data['name'], category_id=cat.id)
        ...
        if task=="add":
            try:
                ...
                return HttpResponse(f"<b> {name1} successfully saved!</b><br><br>{list_all()}")
        elif task=="delete":
            try:
...
class test(View):
    def get(self,request):
        return render(request,'test.html',context={'all':Category.objects.all()})